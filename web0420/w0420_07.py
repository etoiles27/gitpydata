...
# url ="https://browse.auction.co.kr/search?keyword=tv&itemno=&nickname=&encKeyword=tv&arraycategory=&frm=&dom=auction&isSuggestion=No&retry=&k=32&p=4"
...
for i in range(pagecnt):
    url ="https://browse.auction.co.kr/search?keyword=tv&itemno=&nickname=&encKeyword=tv&arraycategory=&frm=&dom=auction&isSuggestion=No&retry=&k=32&p="+str(i)
    res = requests.get(url,headers=headers)
    res.raise_for_status()  
    soup = BeautifulSoup(res.text,"lxml")
